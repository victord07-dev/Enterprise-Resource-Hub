---
title: Sales Return (GST + Credit Note + Ledger + AR)
---
# Sales Return (GST + Credit Note + Ledger + AR)
  ...
  - Attachments (return photos, damaged goods proof) via existing attachments system: entityType="sales_return"
  ...
  3. **Backend routes** — Implement:
     ...
     - POST /api/sales-returns/:id/process — single atomic DB transaction: verify status is "draft" (409 otherwise); validate qtyReturned bounds and > 0 for at least one item; skip service items and non-stock-tracked products for ledger; insert stock_movements with movementType="RETURN_IN", referenceType="SALES_RETURN"; compute credit note totals with correct GST split; insert credit note with CN-{FY}-{seq} numbering; update invoice creditedAmount and recompute status; mark return "processed"; write audit log.
     - GET /api/sales-returns and GET /api/credit-notes — list endpoints with customer name join.

  4. **Sales Invoices UI** — Add "Create Return" button to each expanded invoice row in SalesInvoices.tsx. Build a SalesReturnDialog showing a Return Type selector, per-item table (Sold / Already Returned / Return Qty inputs), a live summary footer (Subtotal / GST Reversal / Total Credit), a Reason field, and a Process button. After processing, show AttachmentsPanel (entityType="sales_return") and display the credit note number inline on the invoice.
