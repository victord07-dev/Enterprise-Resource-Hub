...
import { Download, FileSpreadsheet, FileText, RefreshCw, Truck } from "lucide-react";
  ...
  return (
    <div className="space-y-4" data-testid="panel-supplier-aging">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <Button variant="outline" size="sm" onClick={() => refetch()} className="h-8" data-testid="button-supplier-aging-refresh">
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr><td colSpan={8} className="text-center py-12 text-muted-foreground">No outstanding balances found.</td></tr>
                ) : (
                  filteredRows.map(row => {
                    const bucket = highestBucket(row);
                    return (
                      <tr key={row.supplierId} className="border-b hover:bg-muted/20 transition-colors" data-testid={`row-supplier-aging-${row.supplierId}`}>
                ...
                {!isLoading && filteredRows.length > 0 && (
                  <tr className="border-t-2 bg-blue-50/50 dark:bg-blue-950/10 font-semibold">
                    <td className="px-4 py-2.5">TOTAL ({filteredRows.length} suppliers)</td>
