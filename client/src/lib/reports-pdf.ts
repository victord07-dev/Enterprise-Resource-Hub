...
import { drawLetterhead, fmtPdfPeriod, WARN_GLYPH } from "@shared/pdf-letterhead";
...
// Phase 4C P1 + P6-EXT: every PDF doc gets NotoSans registered so ₹/Δ/em-dash
// render correctly, and setFont calls for "helvetica" are transparently
// redirected to NotoSans. Replaces all inline `await newDoc(...)`.
async function newDoc(opts: any): Promise<jsPDF> {
  const J = await loadJsPDF();
  const doc = new J(opts);
...
function drawHeader(doc: jsPDF, title: string, subtitle: string): number {
  // Delegates to the canonical shared letterhead. Returns the y-coord
  // where body content should start (38 mm = 30 mm header + 8 mm banner
  // + 0 mm padding; callers add their own padding if they want extra).
...
export async function generatePLStatementPDF(d: PLStatementPDFData, charts: PLChartImages = {}): Promise<Blob> {
  ...
  const pwP = 210, mP = 12;
  const periodStr = fmtPdfPeriod(d.period.from, d.period.to);

  // Letterhead (canonical, shared with all PDFs)
  let y = drawLetterhead(doc, {
    pageWidth: pwP,
    margin: mP,
    title: "Profit & Loss Statement",
    bannerSubtitle: periodStr,
  });
...
export async function generateCashFlowStatementPDF(d: CashFlowStatementPDFData): Promise<Blob> {
  ...
  if (d.notes.legacyReceiptsExcluded.count > 0) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(120, 53, 15);
    doc.text(`${WARN_GLYPH} ${d.notes.legacyReceiptsExcluded.count} legacy receipt(s) ${fmtINR(d.notes.legacyReceiptsExcluded.amount)} excluded`, mP + 3, y + 4.5);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.5);
