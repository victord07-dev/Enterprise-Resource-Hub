...
- **Late Arrival Request & Approval System:** Employees submit late arrival requests from My Portal (date, expected time, reason). HR managers and admins review pending requests in the Employees → Late Arrivals tab (approve/reject with note). Approved requests prevent the kiosk auto-marking `half_day` for post-9:35 AM check-ins — attendance is set to `present` and a structured JSON note (`lateArrivalApproved: true`, request ID, time, reason) is stored in `attendance_records.notes`. Reviewers are notified on new submissions; employees are notified on approve/reject via in-app notifications. Schema: `late_arrival_requests` table + `notes` column on `attendance_records`. IST-safe date lookup via `todayIST()`.
...
- **Phase 3 — Operational Refinements:** (A) Stock Adjustment feature-flagged off (`SHOW_STOCK_ADJUSTMENT=false`); `POST /api/stock-movements` requires admin role. (B) Delivery challan workflow extended with `ready → do_issued → dispatched → delivered` statuses, transport fields (`physicalChallanNumber`, `vehicleOwnerName`, `driverPhone`, `doIssuedAt/By`), new backend routes (`ready-for-signature`, `issue-delivery-order`, `upload-signed-copy`, `cancel`), and deep-link `?tab=challans&challanId=UUID`. (C) GRN simplified: `supplierChallanNumber` is now a required creation field; confirm gate removed; server-side GRN PDF (`server/grn-pdf.ts`) served via `GET /api/grns/:id/pdf` with DRAFT/CANCELLED watermarks and FIFO line items. (D) Sales Returns tab hidden from UI (`SHOW_SALES_RETURNS=false`); deep-link `?tab=returns` redirects to `?tab=invoices`. (E) Sales Invoices page redesigned: three summary cards removed, compact 64px list rows with left-border accent, responsive 40/60 split pane with mobile full-screen overlay, detail panel restructured (24px invoice number header, customer card, smart GST columns, prominent 28px grand total, always-visible document status rows, collapsible mark-as-recorded form, external reference section, download buttons).
- **Grid Type (Ticket #78 Phase B):** Products have a `gridType` field (`off_grid` | `on_grid` | `hybrid` | `others`, default `others`). All 248 existing products defaulted to `others` on migration. Editable via product edit form (select field). Included in CSV import/export template (`gridType` column, validated against enum, defaults to `others` on blank/invalid). `HierarchicalProductPicker` (Type→Brand→Product) shows an optional Grid Type filter step between Brand and Product when a brand has products spanning more than one grid type; hidden when all products share the same type (avoids clutter for homogeneous catalogs). Products with `gridType ≠ 'others'` display a `[Off-Grid]`/`[On-Grid]`/`[Hybrid]` badge next to their name in the picker list. Operator upgrades individual products from `others` to a specific grid type via edit form or CSV re-import.
- **Field Visit Out/In Kiosk (Task #86):** `attendance_records` gains nullable `field_visit_out` and `field_visit_in` timestamp columns. Kiosk adds `field_visit_out` and `field_visit_in` as new break-type actions (Building2 / CornerDownLeft icons, teal colour). `field_visit_out` is available any time the employee is checked in and has not already started a field visit (no time-gate). Scanning again when a field visit is open auto-routes to `field_visit_in` (same UX as lunch/tea). Success screen shows teal confirmation with timestamp. Employees.tsx attendance table gains "Field Out" and "Field In" columns; field visit duration is counted in the Break Time total.
- **Phase 4B — Cash & Bank Account Ledger:** `cash_accounts` table (type: bank|cash, openingBalance, bankName, accountNumber, ifscCode, isActive). `account_transfers` and `balance_adjustments` for fund movements and reconciliation. FK `cashAccountId` on `customer_payments`, `supplier_payments`, `expenses`. Startup seed: 4 accounts (HDFC Bank, ICICI Bank, AXIS Bank, CEO Cash). On-the-fly balance via `computeAccountBalance()` SUM over all linked transactions. Accounts page gains admin-only "Cash Position" tab (summary cards: Bank Total, Cash Total, Net Position; per-account clickable cards) and "Cash Accounts" tab (CRUD table with create/edit/deactivate). Payment dialogs (AR, AP, Expense) show smart-filtered account dropdown (cash method → cash accounts; bank/card/UPI/NEFT → bank accounts). Account Detail page (`/accounts/cash-accounts/:id`) shows balance, period stats (totalIn, totalOut, transactionCount), paginated ledger with credit/debit/running-balance columns, Transfer dialog, and Adjustment dialog. Route registered in App.tsx before `/accounts`.
