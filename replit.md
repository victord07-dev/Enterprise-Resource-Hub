...
- **In-App Notifications:** Notifications table (id, userId, type, title, message, isRead, relatedId, createdAt) stores per-user alerts. Bell icon in the top header bar shows unread count badge, opens a popover with the 50 most recent notifications, supports per-notification and mark-all-read. Notifications are also shown as a card in My Portal (5 most recent). Triggers: travel expense approve/reject (notifies the expense owner), payroll disburse (notifies all active employees with a linked user account). Both poll every 60 seconds.
